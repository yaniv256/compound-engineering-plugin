...
| ❌ Wrong | ✅ Correct |
|----------|-----------|
| Subagents write product files into `docs/` or edit tracked paths | Subagents write only scratch artifacts under the resolved owner-scoped `{run_dir}` and return the path; orchestrator writes the one final doc |
